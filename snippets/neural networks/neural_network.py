...
def get_rm_model_mlp(parameters):
    ...
    rmX1 = Dense(2*numDependent, activation='relu', kernel_initializer=rmFmKernelInitializer)(rmInputs)
    rmX2 = Dense(numDependent**2, activation='relu', kernel_initializer=rmFmKernelInitializer)(rmX1)
    rmX3 = Dense(numIndependent**2, activation='relu', kernel_initializer=rmFmKernelInitializer)(rmX2)    
    rmX4 = Dense(2*numIndependent, activation='relu', kernel_initializer=rmFmKernelInitializer)(rmX3)
    rmOutputs = Dense(numIndependent, activation='linear', kernel_initializer=rmFmKernelInitializer)(rmX4)
...
def get_rm_model_variational(parameters):
    ...
    rmX3 = Dense(numIndependent**2, activation='relu', kernel_initializer=rmFmKernelInitializer)(z)    
    rmX4 = Dense(2*numIndependent, activation='relu', kernel_initializer=rmFmKernelInitializer)(rmX3)
    rmOutputs = Dense(numIndependent, activation='linear', kernel_initializer=rmFmKernelInitializer)(rmX4)
    rm = Model(rmInputs, rmOutputs, name='rm')
...
if(rmOnly == True):
    ...
    rmModelFunction = globals()["get_rm_model_" + rmType]
    rmModel,_ = rmModelFunction(parameters)
