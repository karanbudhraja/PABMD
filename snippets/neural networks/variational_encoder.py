...
def get_rm_model(parameters):
    ...
    x = Dense(2*numDependent, activation='relu')(encoderInputs)
    ...
    alpOutputs = Dense(numIndependent, activation='sigmoid')(x)
    fmX1 = Dense(2*numIndependent, activation='relu')(alpOutputs)
    ...
    # now combine
    decoderFmOutputs = decoderFm(encoder(encoderInputs)[2])
    model = Model(encoderInputs, decoderFmOutputs, name='vae_mlp')
    
    # compile model with custom loss
    # the loss must talk about the outputs
    # kl divergence loss + rm/fm reconstruction loss
    klLoss = 1 + zLogVar - K.square(zMean) - K.exp(zLogVar)
    klLoss = K.sum(klLoss, axis=-1)
    klLoss *= -0.5
    klLoss = K.mean(klLoss)
    reconstructionLoss = K.mean(mse(encoderInputs, decoderFmOutputs))

    # just using reconstruction loss for now
    vaeLoss = reconstructionLoss
...
def train_rm_model():
    ...
    estimators = []
    estimators.append(('preprocessing', StandardScaler()))
    estimators.append(('network', KerasRegressor(build_fn=get_rm_model, parameters=[numIndependent, numDependent], epochs=rmEpochs, batch_size=rmBatchSize, verbose=1)))
    pipeline = Pipeline(estimators)
    pipeline.fit(YTrain)
