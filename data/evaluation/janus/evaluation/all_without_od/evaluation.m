...
% without random data
allData = [withSamplingDatasetSelectionPruningData; withoutSamplingData; withoutDatasetSelectionData; withoutPruningData; allPlusOutlierDetectionData];
experimentSize = size(randomData, 1);
samplingPresent = [ones(experimentSize, 1); zeros(experimentSize, 1); ones(experimentSize, 1); ones(experimentSize, 1); ones(experimentSize, 1)];
datasetSelectionPresent = [ones(experimentSize, 1); ones(experimentSize, 1); zeros(experimentSize, 1); ones(experimentSize, 1); ones(experimentSize, 1)];
pruningPresent = [ones(experimentSize, 1); ones(experimentSize, 1); ones(experimentSize, 1); zeros(experimentSize, 1); ones(experimentSize, 1)];
outlierDetectionPresent = [zeros(experimentSize, 1); zeros(experimentSize, 1); zeros(experimentSize, 1); zeros(experimentSize, 1); ones(experimentSize, 1)];
